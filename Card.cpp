...
std::istream & operator>>(std::istream &is, Card &card){
 ...
 string inputSuit = "";
 string temp = "";
 is >> inputRank >> temp >> inputSuit;
 card.rank = string_to_rank(inputRank);
 card.suit = string_to_suit(inputSuit);
 return is;
 //do we need temp to take in of
}
...
bool operator<(const Card &lhs, const Card &rhs){
 ...
 //why does suit matter if trump is disregarded
}
...
else if ((a.is_left_bower(trump) || a.is_right_bower(trump)) && !(b.is_left_bower(trump) || b.is_right_bower(trump))){
 return false;
}
else if ((b.is_left_bower(trump) || b.is_right_bower(trump)) && !(a.is_left_bower(trump) || a.is_right_bower(trump))){
 return true;
}
